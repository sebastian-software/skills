...
`:user-valid` and `:user-invalid` (Baseline 2023) solve the biggest problem with CSS form validation: premature error display. Unlike `:invalid` (which fires immediately, even before the user types anything), these pseudo-classes only activate _after the user has interacted_ with the field.
